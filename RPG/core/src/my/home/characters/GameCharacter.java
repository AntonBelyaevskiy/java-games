...
public abstract class GameCharacter {
    ...
    public void render(SpriteBatch batch, BitmapFont font24) {
        ...
        batch.draw(textureHp, position.x - 42, position.y + 80 - 42 + (int) (Math.sin(animationTimer) * 15 * damageEffectTimer), 84, 16);

        batch.setColor(1, 0, 0, 1);
        batch.draw(textureHp, position.x - 40, position.y + 80 - 40 + (int) (Math.sin(animationTimer) * 15 * damageEffectTimer), 0, 0, hp / hpMax * 80, 12, 1, 1, 0, 0, 0, 80, 20, false, false);
        batch.setColor(1, 1, 1, 1);
        stringHelper.setLength(0);
        stringHelper.append((int) hp);
        font24.draw(batch, stringHelper, position.x - 40, position.y + 80 - 22 + (int) (Math.sin(animationTimer) * 15 * damageEffectTimer), 80, 1, false);
    }
